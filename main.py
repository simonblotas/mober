...
parameters ={
    ...
    "encoded_dim" : 64,
    "n_sources" : 2,
    "src_weights_src_adv" : torch.tensor([0.5, 1.]),
    "kl_weight" : 1e-5,
    "src_adv_weight" : 0.01,
    'batch_size' : 32,
...
ach_final, tcga_final = access_data(ach_data_path,tcga_projects_path,ach_metadata_file_path, tcga_data_path, tcga_metadata_file_path)
...
train_dataloader = DataLoader(combined_dataset, batch_size=parameters['batch_size'], sampler=train_sampler)
val_dataloader = DataLoader(combined_dataset, batch_size=parameters['batch_size'], sampler=val_sampler)
test_dataloader = DataLoader(combined_dataset, batch_size=parameters['batch_size'], sampler=test_sampler)


model_BatchAE = BatchVAE(n_genes=parameters['n_genes'], enc_dim=parameters['encoded_dim'], n_batch=parameters['n_sources'], enc_hidden_layers=(256, 128),enc_dropouts=(0.1, 0.1),dec_hidden_layers =(128, 256) , dec_dropouts = (0.1, 0.1), batch_norm_enc=parameters['batch_norm_enc'], batch_norm_dec=parameters['batch_norm_dec'])
model_BatchAE.to(device)
optimizer_name_BatchAE = parameters['optimizer_name_BatchAE']
optimizer_BatchAE = getattr(optim, optimizer_name_BatchAE)(model_BatchAE.parameters(), lr=parameters['vae_learning_rate'])
#model_src_adv = MLP(enc_dim =parameters['encoded_dim'] ,output_dim=parameters['n_sources'])
model_src_adv = MLP(input_dim=parameters['encoded_dim'], hidden_layers = parameters['mlp_hidden_layers'], output_dim = parameters['n_sources'] , dropouts=parameters['mlp_dropouts'], batch_norm_mlp=parameters['batch_norm_mlp'], softmax_mlp=parameters['softmax_mlp'])
model_src_adv.to(device)
optimizer_name_src_adv = parameters['optimizer_name_src_adv']
optimizer_src_adv = getattr(optim, optimizer_name_src_adv)(model_src_adv.parameters(), lr=parameters['adv_learning_rate'])
