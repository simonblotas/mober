...
def project_into_decoded_space(dataloader, model_BatchAE, device, space_to_project_into='TCGA'):
    all_decoded_data = []
    true_abbreviations = []
    ach_decoded_data = []
    ...
    for batch in dataloader:
        genes, labels, abbreviations = batch[2].to(device), batch[0].to(device), batch[1].to(device)
        ...
        mask_is_in_tcga = torch.all(labels == torch.tensor([0.,1.]).to(device), dim = 1)
        mask_is_in_ach = torch.all(labels == torch.tensor([1.,0.]).to(device), dim = 1)
        ...
        all_decoded_data.append(decoded_data.cpu().detach().numpy())  # Assuming you want to collect the results
        ...
        ach_true_abbreviations.append(abbreviations[mask_is_in_ach].cpu().detach().numpy())
        tcga_decoded_data.append(decoded_data[mask_is_in_tcga].cpu().detach().numpy())
        tcga_true_abbreviations.append(abbreviations[mask_is_in_tcga].cpu().detach().numpy())
    ...
    return all_decoded_data, true_abbreviations, ach_decoded_data, ach_true_abbreviations, tcga_decoded_data, tcga_true_abbreviations
