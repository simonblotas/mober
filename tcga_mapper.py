...
# Mapping between OncotreePrimaryDisease and TCGA_project 'disease'
mappings = [
...
# Create a dictionary where keys are OncotreePrimaryDisease and values are TCGA_project 'disease'
primary = pd.Series(
    df_list["TCGA_project"].values, index=df_list["OncotreePrimaryDisease"].values
).to_dict()

# Get unique double mapped values
double_mapped = list(
...
# Create a dictionary for subtypes
sub_type = {key: None for key in double_mapped}

# Define subtypes for specific diseases
...
# Convert all keys and values to lower case
...
def tcga_mapper(x, sub_x, primary, sub_type):
    if x in sub_type.keys():
        ...
        return "miscellaneous"  # Indicates a mixed disease
